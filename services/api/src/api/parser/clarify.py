...
class ClarifyEngine:
    ...
    async def decide(
        ...
        for candidate in candidates:
            ...
            if base.is_estimate and items[idx].brand:
                # An informed branded label-read (Chobani, Babybel): the user already told
                # us exactly what the product is — asking follow-ups here is the anti-goal
                # (field complaint 2026-07). The estimate's serving is the answer.
                continue
            if attr == "fat_ratio" and base.match_kind is not MatchKind.FAMILY_DEFAULT:
