...
- [ ] R3. Repair fuse: a CAF repair that killed the last process is never retried; the bundle is quarantined with the bytes kept (self-test scenario)
- [ ] R4. Bounds: upload refuses a blob over the server cap before reading it; debug-events.jsonl rotates like observability.jsonl
- [ ] R5. Ladder document; CI rewrite (app compile, voice scenarios, parser corpus, tidy, toolchain printed); `.githooks/pre-push`; wiring rows TIDY-CI/HOOK; dead pre-commit config removed
