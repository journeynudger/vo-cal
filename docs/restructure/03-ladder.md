...
| 6 | Voice runtime scenarios | `bin/ios-sim-voice-test` | ten named scenarios on the pinned simulator with isolated storage roots and structured pass/fail events: golden path, interruption, route change, process death, permission denial, blocked deadline, stall, CAF repair, corrupt-bundle quarantine, repair fuse | a real device and a real microphone; the network | 31 s with the simulator booted, ~2 min cold |
