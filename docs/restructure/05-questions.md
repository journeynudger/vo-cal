...
| 4 | Enable GitHub branch protection on `main` requiring the API, Libraries and iOS app checks? | (a) enable now; (b) leave merges unguarded. | Enabled in R15 if the ship's CI run is green, since "merges have to have some level of verification" was the owner's first priority. | (a); the owner can loosen it in the repository settings. |
